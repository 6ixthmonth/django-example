...
- 데이터베이스를 설정할 때, 다음 변수들 또한 같이 설정하길 권장한다.
  - `LANGUAGE_CODE`
    ...
    - 설정한 언어 코드를 활성화시키려면 `USE_I18N` 변수의 값이 True여야 한다.
  - `TIME_ZONE`
  ...
  - `USE_TZ`
    ...
    - 이 프로젝트에서 날짜-시간 데이터를 조작할 때,
      ...
      - 이 값이 True인 경우,
        ...
        - 표준 시간대에 따라 시간을 변환해서 저장하고,
        - `TIME_ZONE` 설정에 따라 다시 시간을 변환해서 출력한다.
...
- `BooleanField`: 논리 필드. 주로 체크박스 등의 형태로 입력받을 때 사용한다. default 옵션을 설정하지 않으면 False가 아닌 None을 기본 값으로 설정한다.
