...
- `USE_TZ`
  ...
  - 이 값이 `False`인 경우,
    - 이 프로젝트에서 날짜-시간 데이터를 조작할 때, 시간대 정보를 가지지 않는(*timezone-naive*) 객체를 사용하며, 입력한 시간을 그대로 저장하고 출력한다.
  - 이 값이 `True`인 경우,
    - 이 프로젝트에서 날짜-시간 데이터를 조작할 때, 시간대 정보를 가지는(*timezone-aware*) 객체를 사용하며, 표준 시간대에 따라 시간을 변환해서 저장하고 `TIME_ZONE` 설정에 따라 다시 시간을 변환해서 출력한다.
