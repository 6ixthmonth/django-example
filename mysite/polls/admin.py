...
class ChoiceInline(admin.TabularInline):
    """질문 데이터 생성 시 선택지 데이터 또한 함께 생성하기 위한 사용자 정의 인라인 클래스."""
...
class QuestionAdmin(admin.ModelAdmin):
    ...
    # fields = ['pub_date', 'question_text']  # 질문 데이터 생성, 수정 시 다룰 필드들 설정.
    # 질문 데이터 생성, 수정 시 다룰 필드들 상세 설정.
    fieldsets = (
        (None, {  # 첫 번째 필드 그룹 이름.
        ...
        ('Date information', {  # 두 번째 필드 그룹 이름.
            ...
            'classes': ('collapse',),  # 이 그룹을 기본적으로 숨기기 처리.
            'description': 'The date of published.',  # 이 그룹에 대한 부연 설명.
        }),
    )

    inlines = [ChoiceInline]  # 질문 데이터 생성 시 선택지 데이터 또한 함께 생성할 수 있도록 인라인으로 불러온다.

    list_display = ('question_text', 'pub_date', 'was_published_recently')  # 질문 목록에서 표시할 열 설정.
    list_filter = ['pub_date']  # 질문 목록에서 필터를 적용할 열 설정. 설정하면 특정 열의 데이터를 빠르게 필터링 하기 위해 몇 가지 옵션을 제공해준다.
