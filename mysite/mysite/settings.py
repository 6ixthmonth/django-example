...
# 이하의 설정들은 빠른 개발을 돕기 위한 설정들이다. 당연히 실제 서비스하는 제품에 적용하면 안 된다(ALLOWED_HOSTS 변수 제외).
...
# 따로 설정하지 않는 경우 기본 값은 ''이지만, 그러면 프로젝트가 정상적으로 동작하지 않는다.
...
# INSTALLED_APPS: 이 프로젝트에 설치된 앱의 목록. 생성한 앱에 대해 마이그레이션을 적용시키려면 여기에 등록해야 한다.
# 애플리케이션에 대한 상세 내용.
...
# ROOT_URLCONF: 요청 URL을 처리할 때 사용하기 위한, 가장 기본적인 URL 구성(URLconf) 파일의 위치.
# 장고에서 요청을 처리하는 과정.
...
# AUTH_PASSWORD_VALIDATORS: 사용자가 사용하는 비밀번호에 대해 강력한 정도를 검사하는 검증기들의 목록.
...
# 이 설정은 만약 로케일 미들웨어를 사용하지 않는 경우 모든 사용자에게 제공되는 번역 언어를 결정한다.
...
# USE_TZ가 False인 경우, 장고는 시간대 정보를 포함하지 않는(naive) 날짜-시간 객체를 사용한다. 날짜-시간 객체는 서버가 가동되는 지역의 현지 시간을 기준으로 설정된다.
USE_TZ = True
